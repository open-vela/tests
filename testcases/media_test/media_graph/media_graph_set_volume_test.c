/****************************************************************************
 * apps/tests/testcases/media_test/media_graph/media_graph_set_volume_test.c
 *
 * Name: media_graph_set_volume_test
 * Example description:
 *  1. media_graph_set_volume_test
 *
 ****************************************************************************/
...
int main(int argc, FAR char *argv[])
{
  ...
  FUN_CHECK(mediatest_player_set_volume(media), media,
            mediatest_common_close, 0, "FAIL! media set volume fail.\n");
  sleep(3);
  FUN_CHECK(mediatest_player_get_volume(media), media,
            mediatest_common_close, 0, "FAIL! media set volume fail.\n");

  if (abs(value - media->volume) > 0.1)
    {
  ...
  syslog(LOG_INFO, "PASS ! media_graph_set_volume pass \n");
  return 0;
}
