  ...
  while (media->rept--)
    {
      ...
      sleep(3);
      FUN_CHECK(mediatest_common_reset(media), media,
                mediatest_common_close, 1, "FAIL ! media reset fail\n");
    }
  STATE_CHECK(PLAYER_STOPPED, media, mediatest_common_close,
              "player: reset event return failed\n");
  FUN_CHECK(mediatest_common_close(media), media, mediatest_common_close,
            0, "FAIL! media close fail.\n");

  syslog(LOG_INFO,
         "PASS ! media reset pass, media type: %d mode: %d pass \n",
