...
void *media_utils_focus2_test05_thread(void *arg)
{
...
void *media_utils_focus2_test05_initthread(void *arg)
{
  stream_chain_s *chain = (stream_chain_s *)arg;
  int ret = 0;
  chain->handle = media_focus_request2(
...
/*打断+ reply   A：hand repy B：autoreply*/
void test_media_focus2_05(FAR void **state)
{
  ...
  ret = pthread_create(&chain[0].thread, &attr, media_utils_focus2_test05_initthread,
                       (void *)&chain[0]);
  assert_true(ret >= 0);
  sleep(1);
  ret = pthread_create(&chain[1].thread, &attr,
                       media_utils_focus2_test05_thread,
                       (void *)&chain[1]);
