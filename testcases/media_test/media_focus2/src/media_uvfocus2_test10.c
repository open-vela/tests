...
int media_utils_uvfocus2_test10_init(uvfocus2_priv_s *priv)
{
  // int ret = 0;
  priv->chain[0].handle = media_uv_focus_request2(
      priv->loop, priv->chain[0].stream, focus2_uv_change_overtime_callback, 1,
      (void *)&priv->chain[0]);
  return 0;
}

int media_utils_uvfocus2_test10_third(uvfocus2_priv_s *priv)
{
  // int ret = 0;
...
void test_media_uvfocus2_10(FAR void **state)
{
  ...
  UV_FOCUS2_EXECUTE(media_utils_uvfocus2_test10_init, priv);
  UV_FOCUS2_EXECUTE(media_utils_uvfocus2_test10_third, priv);
