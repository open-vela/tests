...
static void *media_utils_focus2_test07_thread(void *arg)
{
...
static void *media_utils_focus2_test07_initthread(void *arg)
{
  stream_chain_s *chain = (stream_chain_s *)arg;
  int ret = 0;
  chain->handle = media_focus_request2(
...
/*Interrupt + reply   A: hand reply + Timeout B: auto reply*/
void test_media_focus2_07(FAR void **state)
{
  ...
  ret = pthread_create(&chain[0].thread, &attr,
                       media_utils_focus2_test07_initthread,
                       (void *)&chain[0]);
  assert_true(ret >= 0);
  sleep(1);
  ret = pthread_create(&chain[1].thread, &attr,
                       media_utils_focus2_test07_thread,
                       (void *)&chain[1]);
