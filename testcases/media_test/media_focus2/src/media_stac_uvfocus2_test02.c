...
int media_utils_uvfocus2_test02_init(uvfocus2_priv_s *priv)
{
  // int ret = 0;
  priv->chain[0].handle = media_uv_focus_request2(
      priv->loop, priv->chain[0].stream, focus2_uv_change_reply_callback,
      0, (void *)&priv->chain[0]);
  return 0;
}
  ...
  for (int i = 0; i < 300; i++)
    {
      ...
      UV_FOCUS2_EXECUTE(media_utils_uvfocus2_test02_init, priv);
      ...
      priv->chain[0].handle = NULL;
      priv->chain[0].ret = -1;
      priv->chain[0].ready = false;
      priv->chain[0].nb_suggests = 0;
      priv->chain[0].isabandoned = false;
      priv->chain[0].type = 1;
      priv->chain[0].cookie = priv;
      memset(priv->chain[0].suggests, 0,
             sizeof(priv->chain[0].suggests));
    }
}