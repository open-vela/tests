...
static void *media_utils_focus2_test12_thread(void *arg)
{
  stream_chain_s *chain = (stream_chain_s *)arg;
  int ret = 0;
  chain->handle =
      media_focus_request2(&ret, chain->stream,
                           focus2_change_multi_reply_callback, 0, (void *)chain);
...
void test_media_focus2_12(FAR void **state) {
  ...
  ret = pthread_create(&chain[0].thread, &attr,
                       media_utils_focus2_test12_thread, (void *)&chain[0]);
  assert_true(ret >= 0);
