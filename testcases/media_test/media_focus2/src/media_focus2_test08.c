...
static void *media_utils_focus2_test08_thread(void *arg)
{
...
static void *media_utils_focus2_test08_initthread(void *arg)
{
  stream_chain_s *chain = (stream_chain_s *)arg;
  int ret = 0;
  chain->handle = media_focus_request2(
      &ret, chain->stream, focus2_change_delay_reply_callback, 0,
      (void *)chain);
  assert_non_null(chain->handle);
  mediatest_trace_suggest(chain, ret);
  if (ret != MEDIA_FOCUS_PLAY)
...
/*打断+ reply   A： hand reply + 超时 B：handle*/
void test_media_focus2_08(FAR void **state)
{
  ...
  ret = pthread_create(&chain[0].thread, &attr,
                       media_utils_focus2_test08_initthread,
                       (void *)&chain[0]);
  assert_true(ret >= 0);
  sleep(1);
  ret = pthread_create(&chain[1].thread, &attr,
                       media_utils_focus2_test08_thread,
                       (void *)&chain[1]);
