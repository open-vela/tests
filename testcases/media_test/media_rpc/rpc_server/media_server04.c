/****************************************************************************
 * tests/testcases/media_test/media_rpc/rpc_server/media_server04.c
 *
...
static void *server_test4(void *server_handle)
{
    ...
    while (1) {
        ...
        if (index == 2) {
            if (f)
                syslog(LOG_ERR, "PASS! media_server04 pass.\n");
            break;
        }
        ...
        for (int i = 0; i < count; i++) {
            if (fds[i].revents) {
                if (conns[i] == NULL) {
                    ...
                    ret = media_server_poll_available(server_handle, &fds[i], conns[i]);
                    syslog(LOG_INFO, "conns is not null, test recv() %d\n", ret);
                    if (ret != 0) {
                        f = 0;
                        syslog(LOG_ERR, "FAIL! test recv(), ret = %d.\n", ret);
                        return NULL;
                    }
                }
            }
...
Description:
    1. Test the conns parameter of function media_server_poll_available().
    2. Test conns is null and conns is not null.
    3. When conns is null, will test accept().
    4.Whend conns is not null, will test recv().
...
int main(int argc, FAR char *argv[])
{
    ...
    pthread_create(&server_thread, NULL, server_test4, server_handle);
