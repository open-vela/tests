/****************************************************************************
 * tests/testcases/media_test/media_rpc/rpc_server/media_server05.c
 *
...
static void *server_test5(void *server_handle)
{
    ...
    while (1) {
        ...
        if (index == 2) {
            if (f)
                syslog(LOG_ERR, "PASS! media_server05 pass.\n");
            break;
        }
        ...
        for (int i = 0; i < count; i++) {
            if (fds[i].revents) {
                if (conns[i] == NULL) {
                    ...
                    ret = media_server_poll_available(NULL, &fds[i], conns[i]);
                    syslog(LOG_INFO, "handle is not null, handle is null, test recv(),ret = %d\n", ret);
                    if (ret!= -EINVAL) {
                        f = 0;
                        syslog(LOG_ERR, "FAIL! test recv(), handle is null, should fail.\n");
                        return NULL;
                    }
                }
            }
...
Description:
    1. Test the handle parameter of function media_server_poll_available().
    2. Handle is null.
...
int main(int argc, FAR char *argv[])
{
    ...
    pthread_create(&server_thread, NULL, server_test5, server_handle);
