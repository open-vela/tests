...
void mediatest_uv_music_record(FAR void **state)
{
  ...
  chain->mediatest_info[1].stream_type = MEDIA_SOURCE_MIC;
  chain->mediatest_info[1].url =
      CONFIG_MEDIATEST_COMMON_PATH CONFIG_MEDIATEST_RECORDER_PATH;
  chain->mediatest_info[1].cookie = &chain->mediatest_info[1];
  chain->mediatest_info[1].focus = MEDIA_SCENARIO_RECORD;
  chain->mediatest_info[1].ret_duration = 5000;
