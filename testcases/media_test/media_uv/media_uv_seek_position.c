...
int main(int argc, FAR char *argv[])
{
  ...
  sleep(media->time);
  unsigned int cur_position = media->position;
  UV_EXECUTE(mediatest_seek, media);
  ST_CHECK(PLAYER_SEEKED, media, "FAILED ! uv player seek failed\n");
  UV_EXECUTE(mediatest_position, media);
  UV_WAIT(PLAYER_GET_POSITION, media,
          "FAILED ! uv player get position failed\n");
  int diff = abs(media->position - cur_position);
  syslog(LOG_INFO, "position diff is %d\n", diff);
  if (diff > 2000)
    {
      goto out;
    }
  syslog(LOG_INFO, "media uvplayer seek position PASSED!\n");
out:
