...
int main(int argc, FAR char *argv[])
{
  ...
  while (--media->rept > 0)
    {
      ...
      usleep(500 * 1000);
      UV_EXECUTE(mediatest_app_reset, media);
      ST_CHECK(PLAYER_STOPPED, media,
               "FAILED ! uv player reset failed\n");
  ...
  usleep(500 * 1000);
  UV_EXECUTE(mediatest_app_reset, media);
  ST_CHECK(PLAYER_STOPPED, media, "FAILED ! uv player reset failed\n");
  syslog(LOG_INFO, "media uvplayer reset PASSED!\n");
