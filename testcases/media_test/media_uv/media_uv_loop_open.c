  ...
  for (i = 0; i < streams_len; i++)
    {
      ...
      for (int j = 0; j < media->rept; j++)
        {
          UV_EXECUTE(mediatest_common_open, medias[i]);
          RET_CHECK(media->ret, "mediatest open failed\n");
          if (media->ex == 1)
            {
              UV_EXECUTE(mediatest_common_prepare, medias[i]);
            }
          else if (media->ex == 2)
            {
              UV_EXECUTE(mediatest_common_prepare, medias[i]);
              UV_EXECUTE(mediatest_common_start, medias[i]);
            }
          UV_EXECUTE(mediatest_app_exit, medias[i]);
          RET_CHECK(media->ret, "mediatest close failed\n");
