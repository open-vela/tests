...
int main(int argc, FAR char *argv[])
{
  ...
  for (i = 0; i < streams_len; i++)
    {
      int value = (int)medias[i]->volume;
      UV_EXECUTE(mediatest_uv_policy_set_stream_volume, medias[i]);
      UV_EXECUTE(mediatest_uv_policy_get_stream_volume, medias[i]);
      UV_WAIT(PLAYER_GET_VOLUME, medias[i],
              "FAILED ! uv player get volume failed\n");
      if (value != (int)medias[i]->volume)
        {
          syslog(LOG_ERR, "get Volume not match set volume\n");
          goto out;
        }
    }

  syslog(LOG_INFO, "TEST PASS! media_uv_policy_volume pass.\n");
