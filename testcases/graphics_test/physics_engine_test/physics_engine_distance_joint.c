...
static inline void create_anchor(anim_engine_handle_t instance, physics_node_t *node)
{
    ...
    */
    ...
    // info.position.y = 332.0f;

    // ANIMID anchor_body = anim_create_body(instance, &info);
    // anim_physics_material_t material_anchor;;
    // anim_get_material(anim_instance, anchor_body, &material_anchor);
    // material.density = 1.0f;
    // material.restitution = 0.3f;
    // anim_set_material(instance, anchor_body, &material_anchor);
...
static inline void create_distance_joint(void)
{
    ...
    l2.x = 232.0f;
    l2.y = 282.0f;
    printf("func create_distance_joint's body_anchor: %lld\n", body_anchor);
    printf("func create_distance_joint's body_box: %lld\n", body_box);
    anim_joint_t joint_body;
    joint_body.type = ANIM_JOINT_DISTANCE;
    joint_body.body1 = body_anchor;
    joint_body.body2 = body_box;
    joint_body.anchor1 = l1;
    joint_body.anchor2 = l2;

    static anim_distance_joint_t distance_joint_data;
    distance_joint_data.base = joint_body;
    distance_joint_data.length = 150.0f;
    distance_joint_data.min_length = 100.0f;
    distance_joint_data.max_length = 200.0f;
    // distance_joint_data.damping = 0.5f;
    // distance_joint_data.stiffness = 0.5f;
    distance_ret = anim_create_joint_distance(anim_instance, &distance_joint_data);
    printf("distance_ret = %d\n", distance_ret);
}
...
//     printf("l2.x = %f\n", l2.x);
//     printf("l2.y = %f\n", l2.y);
//     revolute_joint_data.lower_angle = 100.0f;
//     revolute_joint_data.upper_angle = 200.0f;
//     revolute_joint_data.motor_speed = 90.0f;
//     revolute_joint_data.max_motor_torque = 100.0f;
