...
static inline void create_circles_test(anim_engine_handle_t instance, physics_node_t *node, float friction)
{
    /*
        Create an circle, restitution is 0;
    */
    node->body_info.position.x = 182.0f;
    node->body_info.position.y = 102.0f;
    ...
    node->material.restitution = 0.1;
    node->material.friction =friction;
    node->material.density = 20.0f;
...
static inline ANIMID create_chain_shape_top(anim_engine_handle_t instance, float density)
{
    /* Create top bevel shape*/

    anim_body_info_t info;
    anim_body_init(&info);
    info.position.x = 232.0f;
    info.position.y = 232.0f;
    ...
    // pts[3].x = 252.0f;
    // pts[3].y = 212.0f;
    ANIMID body = anim_create_body_edge_tow_sided(instance, &info, &p1, &p2);
    anim_physics_material_t material_background;
    anim_get_material(anim_instance, body, &material_background);
    material_background.density = density;
    material_background.restitution = 0.1f;
...
static inline ANIMID create_chain_shape_middle(anim_engine_handle_t instance, float density)
{
    /* Create top bevel shape*/
...
static inline ANIMID create_chain_shape_end(anim_engine_handle_t instance, float density)
{
    /* Create top bevel shape*/
...
static void init_physics_engine(void)
{
    ...
    create_chain_shape_top(anim_instance, 1.0);
    create_chain_shape_middle(anim_instance, 1.0);
    create_chain_shape_end(anim_instance, 1.0);
}
