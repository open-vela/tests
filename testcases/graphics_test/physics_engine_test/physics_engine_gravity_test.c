...
{
    anim_vector2f_t linear_v_circle;
    //linear_v_circle.x = 0.5f;
    //linear_v_circle.y = 0.5f;
    linear_v_circle.x = 0.0f;
    linear_v_circle.y = 0.0f;
    init_lvgl_image(&icon_nodes[0], NULL, create_circle_shape, -45, 3.0, linear_v_circle);
}
...
int anim_physics_gravity_test(int argc, char *argv[])
{
    /*
        @brief: Test the animation effect of a round object with a density of 0.3f under different gravitational accelerations
        command: graphics_test physics_gravity {index} &
    */
    anim_vector2f_t positions[] = {
        {9.81f, 9.81f},       // index = 0; diagonal line gravity, animation bit at bottom right
        {-9.81f, -9.81f},     // index = 1; diagonal line gravity, animation bit at top left
        {9.81f, -9.81f},      // index = 2; diagonal line gravity, animation bit at top right
        {-9.81f, 9.81f},      // index = 3; diagonal line gravity, animation bit at bottom left
        {0, 9.81f},         // index = 4; standard earth gravity vertical
        {0, -9.81f},        // index = 5; standard earth gravity reverse vertical
        {9.81f, 0},         // index = 6; horizontal gravity
        {-9.81f, 0},        // index = 7; horizontal gravity reverse
        {0.0001f, 0.0001f}, // index = 8;
        {0, 0}              // index = 9;
    };
    int index = -1;
    if (argv[2] != NULL)
    {
        ...
        // Check if num is within the legal range.
        if (num >= 0 && num < sizeof(positions) / sizeof(positions[0]))
        {
            index = num;
        }
        else
    ...
    if (index != -1)
    {
        vPos = positions[index];
        init_physics_engine();
        printf("vPos.x = %f, vPos.y = %f\n", vPos.x, vPos.y);
    }
    else
    {
