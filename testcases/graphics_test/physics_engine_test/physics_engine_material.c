...
anim_physics_material_t vMaterial;
...
static inline void init_lv_image_obj(void)
{
    ...
    init_lvgl_image(&icon_nodes[0], NULL, create_circle_shape1, 45, 3.0, linear_v);
    // init_lvgl_image(&icon_nodes[1], NULL, create_circle_shape4, -45, 3.0, linear_v_circle);
...
int anim_physics_material_test(int argc, char *agrv[])
{
    ...
    anim_physics_material_t;
    */
    anim_physics_material_t material[] = {//Density cannot be negative, Friction Force ideal status is 0,
        {0.91f, 0.12f, 0.82f},       // index = 0; Density is 0.91, bounce Force is 1.12, Friction Force is 0.82 normal Value
        {0.00f, 0.00f, 0.00f},     // index = 1; Density: 0.00, bounce Force: 0.00, Friction Force is 0.00 Friction Force is ideal status Value 
        {9.81f, 9.81f, 9.81f},     // index = 2; according to float maximum Value 9.81f
    };
    int index = -1;
    if (agrv[2] != NULL)
    {
        ...
        if (num >= 0 && num < sizeof(material) / sizeof(material[0]))
        {
            index = num;
        }
        else
    ...
    if (index != -1)
    {
        vMaterial = material[index];
        ui_screensub_init();
        printf("vMaterial.density = %f, vMaterial.restitution = %f, vMaterial.friction %f\n", vMaterial.density, vMaterial.restitution, vMaterial.friction);
    }
    else
    {
