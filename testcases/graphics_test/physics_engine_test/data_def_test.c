...
static inline ANIMID create_rigid_body_edge_polygon_4_sides(anim_engine_handle_t instance, physics_node_t *node)
{
    ...
    */
    ...
    const int k_segments = 4;                                // 正6边形的边数
    const float r = 20.0f;                                    // 正6边形的半径
    const float k_increment = (2.0f * 3.14159f) / k_segments; // 每个角的弧度增量
...
void create_rigid_body_polygon_17_sides(anim_engine_handle_t instance, physics_node_t *node)
{
    ...
    */
    ...
    const int k_segments = 17;                                // 正17边形的边数
    const float r = 40.0f;                                    // 正17边形的半径
    const float k_increment = (2.0f * 3.14159f) / k_segments; // 每个角的弧度增量
...
void create_material1_rigid_body(anim_engine_handle_t instance, physics_node_t *node)
{
    ...
    */
    ...
    anim_vector2f_t pts[4]; // 正方形有4个顶点
    const int k_segments = 4; // 正方形有4个顶点
    const float k_increment = 2.0f * 3.14159f / k_segments; // 正方形只有一个周长，所以不需要细分
    float r = 60.0f; // 假设半径为40

    // 计算正方形的顶点
    for (int i = 0; i < k_segments; ++i) {
        // 正方形的顶点坐标可以通过将半径分为两部分来计算
        pts[i].x = cosf(-k_increment * i) * r;
        pts[i].y = sinf(-k_increment * i) * r;
        printf("顶点 %d: (%.2f, %.2f)\n", i + 1, pts[i].x, pts[i].y);
    }
