...
static void label2_scale(lv_event_t * gesture_event);
static void label_move(lv_event_t * event);
static void label2_move(lv_event_t * event);
...
static float label2_width;
static float label_height;
static float label2_height;
static float label_x;
static float label2_x;
static float label_y;
static float label2_y;
...
void test_example_gestures(void)
{
    ...
    label2_width = RECT_INIT_WIDTH;
    label2_height = RECT_INIT_HEIGHT;
    label2_y = label2_x = 400;
    ...
    lv_label_set_text(label, "Zoom or move1");
    ...
    // label2
    label2 = lv_label_create(root_view);
    lv_obj_remove_flag(root_view, LV_OBJ_FLAG_SCROLLABLE);

    lv_label_set_text(label2, "Zoom or move 2");
    lv_obj_add_flag(label2, LV_OBJ_FLAG_CLICKABLE);

    lv_style_init(&label2_style);
    lv_style_set_bg_color(&label2_style, lv_color_hex(RECT_COLOR));
    lv_style_set_bg_opa(&label2_style, LV_OPA_COVER);

    lv_style_set_width(&label2_style, (int)label2_width);
    lv_style_set_height(&label2_style, (int)label2_height);

    lv_style_set_x(&label2_style, (int)label2_x);
    lv_style_set_y(&label2_style, (int)label2_y);

    lv_obj_add_style(label2, &label2_style, LV_STATE_DEFAULT);

    lv_obj_add_event_cb(label2, label2_scale, LV_EVENT_GESTURE, label2);
    lv_obj_add_event_cb(label2, label2_move, LV_EVENT_PRESSING, label2);
}
...
static void label_move(lv_event_t * event)
{
    ...
    LV_LOG_USER("label move x: %ld, y: %ld", pnt.x, pnt.y);
...
/**
 * Called when a pinch event occurs - scales the second label
 * @param gesture_event point to a LV_EVENT_PINCH event
 */
static void label2_scale(lv_event_t * gesture_event)
{
    ...
    if(state == LV_INDEV_GESTURE_STATE_ENDED) {
        ...
        LV_LOG_USER("label2 end scale: %g, state: %d", scale, state);
        return;
    }
    ...
    if(initial_h == -1 || initial_w == -1) {
        ...
        /* Pinch gesture has been recognized - this is the first event in a series of recognized events */
        /* The scaling is applied relative to the original width/height of the rectangle */
        initial_w = (int)label2_width;
        initial_h = (int)label2_height;

        LV_LOG_USER("label2 start scale: %g", scale);
    }
    ...
    label2_width = initial_w * scale;
    label2_height = initial_h * scale;
    label2_x = center_pnt.x - label2_width / 2;
    label2_y = center_pnt.y - label2_height / 2;

    LV_LOG_USER("label2 scale: %g label2 x: %g label2 y: %g w: %g h: %g",
                scale, label2_x, label2_y, label2_width, label2_height);
...
/**
 * Called when a LV_EVENT_PRESSING occurs on the second rectangle - moves the label
 * @param event pointer to the event
 */
static void label2_move(lv_event_t * event)
{
    ...
    LV_LOG_USER("label2 move x: %ld, y: %ld", pnt.x, pnt.y);

    label2_x = pnt.x - label2_width / 2;
    label2_y = pnt.y - label2_height / 2;
