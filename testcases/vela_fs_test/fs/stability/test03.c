...
#define TEST_FILE_NAME "stability_test03_file"
...
static uint32_t crc32(const uint8_t *src, size_t len)
{
...
static void get_random_string(char *buf, int length)
{
    int flag;
...
static int crc_check(const char *fileName)
{
    FILE *fp = NULL;
    uint32_t crc = 0;
    struct data_block_info info;
    size_t ret;
    fp = fopen(fileName, "r");
    if (fp == NULL)
    {
    ...
    while (!feof(fp)) /*Determine whether End, there will be a FileEnd character here*/
    {
        ret = fread(&info, sizeof(struct data_block_info), 1, fp);
        if (ret > 0)
        {
            if (info.len == 1024)
                crc = crc32((uint8_t *)(info.data), info.len);
            else
                syslog(LOG_ERR, "bad info.len %zu !\n", info.len);
            if (crc != info.crc)
            {
                syslog(LOG_ERR, "bad crc !\n");
                fclose(fp);
                return ERROR;
            }
        }
...
static int do_write(char *fileName, struct data_block_info *dataBlock)
{
    FILE *fp = NULL;
    fp = fopen(fileName, "a+");
    if (fp == NULL)
    ...
    fwrite(dataBlock, sizeof(struct data_block_info), 1, fp);
    fclose(fp);
    return OK;
}
...
int main(int argc, FAR char *argv[])
{
    ...
    if (argc >= 2)
    {
        ...
        syslog(LOG_INFO, "The test mode is [%s]\n", mode);
    }
    else
    {
        syslog(LOG_WARNING, "Usage:\n");
        syslog(LOG_WARNING, "cmd  mode=<reboot/crash> [path]\n");
        exit(1);
    }
    ...
    if (is_file_exist(TEST_FILE_NAME) == OK)
    {
        syslog(LOG_INFO, "file is exit !\n");
        if (crc_check(TEST_FILE_NAME) == OK)
        {
            syslog(LOG_INFO, "check crc OK !  \n");
            unlink(TEST_FILE_NAME);
            syslog(LOG_INFO, "TEST PASS !\n");
        }
        else
        {
            syslog(LOG_INFO, "check crc FAIL !  \n");
            unlink(TEST_FILE_NAME);
    ...
    else
    {
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            syslog(LOG_INFO, "NO.%d write to the filesystem .\n", i + 1);
            get_random_string(file.data, file.len);
            file.crc = crc32((uint8_t *)(file.data), file.len);
            do_write(TEST_FILE_NAME, &file);
            /* Randomly trigger a restart */
            srand(i);
            /* At least half of the test */
            if (i > (rand() % 500 + 400))
            {
                /* Actively simulate a system reboot */
